...
int label_incrementa = 1; //incrementa ifs e prints
int while_incrementa = 1; //incrementa os whiles
int incrementa; //contador de operacoes
int string_valor;//contador de string
int main_contador = 0; //verificar se existe main ou nao
int entry = 0;
...
void iterate_class(struct node *node, table_element *table){
    if(strcmp(node->type, "Program") == 0){
        ...
        if(entry == 1){
            create_entry();//cria a main entry
        }
        if(main_contador == 0){
            create_main();//cria a main se nao existir
        }
    }
        ...
        if(table->funcdecl->n_params != 0){
            create_methodbody(node->childs[1], table->funcdecl->vars);//preenche a func com tabela normal
        }else{
            create_methodbody(node->childs[1], global_table);//percorre a func com a tabela global
        }
        create_return(node->childs[0]);//adiciona o return 0 no final
        printf("}\n");
    } 
}
...
void create_entry(){
    /* comandos para criar a main, se for necessario usar ags.length*/
...
void create_func(struct node *node){
      ...
      //cria as funcoes do estilo @method_nome_tipo_parametrosdafuncao, exceto a main
    int main_found = 0;
    ...
    printf(") {\n");
    //inicia as variaveis dos parametros
    for(int i = 0 ; i < methodparams->index_childs;i++){
...
void create_methodbody(struct node *node, table_element *table){
    //inicia as vardecls, se for doutro tipo entra nas statements
...
// Cria as linhas de codigo do LLVM para as statements
    ...
    if (strcmp(aux->type, "Assign") == 0){//filho0 = id; filho 1 -> expr 
        ...
        if(strcmp(aux->annotation, "int") == 0){
            ...
            if(!verifica_local(aux->childs[0], table) && verifica_global(aux->childs[0])){
                printf("store i32 %%.%d, i32* @%s_int\n",incrementa-1,aux->childs[0]->value);//var global
                //incrementa++;
            }else{
            ...
            if(strcmp(aux->childs[1]->annotation, "int")==0){
                printf("%%.%d = sitofp i32 %%.%d to double\n",incrementa, incrementa-1);//paassa de int para double de maneira a adicionar corretamente
                incrementa++;
            }
            if(!verifica_local(aux->childs[0], table) && verifica_global(aux->childs[0])){
        ...
        if(aux->index_childs > 1){
            ...
            //percorre os filhos e guarda os valores do ultimo contador usado em cada, de modo a chamar a frente a posicao correta deles
            ...
            char * aux_string = (char *)malloc((tamanho) * sizeof(char *)); //devido a anotacao ser (double, int ....), skippar as virgulas e os parentesis
            int conta = 0;
            int array_ind = 1;
            for(int i = 1 ; i < tamanho-1;i++){
                if(aux->childs[0]->annotation[i] == ','){
                    if(aux->childs[array_ind]->annotation == NULL){
                        //se parametro for double, passar o inteiro para double
                        if(strcmp(aux->childs[array_ind]->type, "DecLit") == 0){
                            ...
                            array_ind++;
                        }else if(strcmp(aux->childs[array_ind]->type, "RealLit") == 0){//aqui nao vai ser necessario eventualmente, mas caso fosse double passar para inteiro, o que e errado
                        ...
                        if(strcmp(aux->childs[array_ind]->annotation,"int") == 0){//passar de inteiro para double caso o parametros da funcao seja double
            ...
            //caso o parametro da funcao seja double passar para inteiro
            if(aux->childs[array_ind]->annotation == NULL){
                if(strcmp(aux->childs[array_ind]->type, "DecLit") == 0){
                    if(strcmp(aux_string, "double") == 0){
                        printf("%%.%d = sitofp i32 %%.%d to double\n",incrementa, array[array_ind-1]);
            ...
            //chama a funcao de acordo com @method_nome_tipo
            if(strcmp(aux->annotation,"int") == 0){
            ...
            memset(&aux_string[0], 0, sizeof(aux_string));
            conta = 0;
            //adiciona os parametros no nome de maneira a chamar corretamento a funcao
            for(int i = 1 ; i < tamanho-1;i++){
            ...
            //parametros que ficam dentro dos parentesis
            if(strcmp(aux_string,"int") == 0){
                printf("i32 %%.%d",array[array_ind-1]);
                array_ind++;
            }else if(strcmp(aux_string,"double") == 0){
            ...
            //caso a funcao nao tenha parametros, chama so @method_nome__tipo
                if(strcmp(aux->annotation,"int") == 0){
                    printf("%%.%d = call i32 @method_%s_int",incrementa,aux->childs[0]->value);
                    incrementa++;
                }else if(strcmp(aux->annotation,"double") == 0){
        ...
        /*segue a documentacao dos ifs 
            %.9 = load i32, i32* %input
            %.10 = add i32 0, 10
            %.11 = icmp slt i32 %.9, %.10
            br i1 %.11, label %then1, label %else1 if(variaveis) -> decide o path
            then1: (entra no if)
        ...
        */
        ...
        /*
            ...
            %.1 = add i32 0, 0
            store i32 %.1, i32* %k
            br label %while2 while(expressao)-> salto
            while2:->statements dentro do while
        ...
        */
        ...
        //vai buscar o (+numero) de args[numero] e torna positiva
        printf("%%.%d = add i32 1, %%.%d\n",incrementa, incrementa-2);
        incrementa++;
        //vai buscar em args a posicao 
        printf("%%.%d = getelementptr i8*, i8** %%.%d, i32 %%.%d\n",incrementa, incrementa-2,incrementa-1);
        incrementa++;
        //guard o valor que se encontra na args[posicao]
...
// Crias as expressions para escrever no LLVM
